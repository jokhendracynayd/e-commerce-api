...
export class PaymentEncryptionUtil {
    ...
    if (!key) {
      ...
      try {
        ...
        if (this.encryptionKey.length !== 32) {
          throw new Error(
            `Invalid key length: ${this.encryptionKey.length} bytes. Expected: 32 bytes.`,
          );
        }
      } catch (error) {
        const errorMsg =
          'Invalid PAYMENT_ENCRYPTION_KEY format. Must be a valid hex string representing 32 bytes (256 bits).';
        if (isProduction) {
          ...
          console.warn(
            'Using randomly generated key instead. DO NOT use in production!',
          );
        }
      }
    }
  ...
  encrypt(data: Record<string, any>): string {
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to encrypt sensitive payment data',
      );
    }
  }
  ...
  decrypt(encryptedString: string): Record<string, any> {
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to decrypt sensitive payment data',
      );
    }
  }
