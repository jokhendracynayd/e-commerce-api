...
export class CartsService {
      ...
      return await this.prismaService.$transaction(
        async (prisma) => {
          ...
          if (existingCartItem) {
            ...
            // Update the existing cart item using Prisma's native operations
            ...
            // Create a new cart item with reservation using Prisma's native operations
  ...
  async mergeAnonymousCart(
    ...
    try {
      this.logger.log(
        `Merging anonymous cart for user ${userId} with ${anonymousCartItems.length} items`,
      );
      ...
      return await this.prismaService.$transaction(
        async (prisma) => {
          ...
          const validatedItems = await Promise.all(
            anonymousCartItems.map(async (item) => {
              try {
                ...
                if (!product) {
                  this.logger.warn(
                    `Product with ID ${item.productId} not found during cart merge validation`,
                  );
                  return {
                    valid: false,
                ...
                if (!product.isActive) {
                  this.logger.warn(
                    `Product with ID ${item.productId} is not active during cart merge validation`,
                  );
                  return {
                    valid: false,
                ...
                if (item.variantId) {
                  ...
                  if (!variant) {
                    this.logger.warn(
                      `Variant with ID ${item.variantId} not found during cart merge validation`,
                    );
                    return {
                      valid: false,
                  ...
                  if (variant.productId !== item.productId) {
                    this.logger.warn(
                      `Variant with ID ${item.variantId} does not belong to product with ID ${item.productId} during validation`,
                    );
                    return {
                      valid: false,
                      message: `Variant with ID ${item.variantId} does not belong to product with ID ${item.productId}`,
                    };
                  }
                ...
                if (adjustedQuantity <= 0) {
                  this.logger.warn(
                    `Not enough stock for product ${item.productId} during cart merge validation`,
                  );
                  return {
                    valid: false,
                ...
                this.logger.error(
                  `Error validating item ${item.productId}: ${error.message}`,
                );
                return {
                  valid: false,
                  message: `Error validating item: ${error.message}`,
                };
              }
            }),
          );
          ...
          if (invalidItems.length > 0) {
            this.logger.warn(
              `${invalidItems.length} items failed validation during cart merge: ${JSON.stringify(invalidItems)}`,
            );
          }
