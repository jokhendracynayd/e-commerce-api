...
export class UserProfilingService {
  ...
  async updateUserProfiles(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Error in user profiling: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async updateUserProfile(userId: string): Promise<UserProfile> {
    ...
    try {
      ...
      const behavioralSegment = await this.calculateBehavioralSegment(
        userId,
        activityData,
      );
      ...
      const lifetimeValue = await this.predictLifetimeValue(
        userId,
        activityData,
      );
      ...
      const sessionMetrics = await this.analyzeSessionMetrics(
        userId,
        activityData,
      );
  ...
  private async calculateBehavioralSegment(
    ...
    const views = activityData.filter(
      (a) => a.activityType === UserActivityType.PRODUCT_VIEW,
    );
    const cartAdds = activityData.filter(
      (a) => a.activityType === UserActivityType.ADD_TO_CART,
    );

    // Calculate ratios
    const purchaseToViewRatio =
      views.length > 0 ? purchases.length / views.length : 0;
    const cartToViewRatio =
      views.length > 0 ? cartAdds.length / views.length : 0;
    const avgSessionDuration =
      this.calculateAverageSessionDuration(activityData);

    // Segment logic
    if (purchases.length === 0 && views.length > 20) {
      return 'browser';
    } else if (purchaseToViewRatio > 0.1 && avgSessionDuration < 300) {
      // 5 minutes
      return 'impulse_buyer';
    } else if (avgSessionDuration > 1800 && cartToViewRatio < 0.1) {
      // 30 minutes
      return 'researcher';
  ...
  private async buildPreferenceVector(
    userId: string,
    activityData: any[],
  ): Promise<UserProfile['preferenceVector']> {
    ...
    for (const activity of activityData) {
        ...
        if (product) {
          ...
          if (product.categoryId) {
            categories[product.categoryId] =
              (categories[product.categoryId] || 0) + weight;
          }
      ...
      if (activity.deviceType) {
        devicePreferences[activity.deviceType] =
          (devicePreferences[activity.deviceType] || 0) + 1;
      }
    }
  ...
  private calculateEngagementScore(activityData: any[]): number {
    ...
    const uniqueDays = new Set(
      activityData.map((a) => a.createdAt.toDateString()),
    ).size;
    ...
    const engagementScore =
      activityFrequency * 0.3 + highValueRatio * 0.4 + recency * 0.3;
  ...
  private async predictLifetimeValue(
    ...
    const daysSinceFirstPurchase = Math.max(
      1,
      (Date.now() - purchases[0].createdAt.getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  private async assessChurnRisk(
    ...
    const lastActivity = Math.max(
      ...activityData.map((a) => a.createdAt.getTime()),
    );
    ...
    if (purchases.length > 0) {
      const daysSinceLastPurchase =
        (Date.now() -
          Math.max(...purchases.map((p) => p.createdAt.getTime()))) /
        (1000 * 60 * 60 * 24);
  ...
  private async updateUserSegments(): Promise<void> {
    ...
    const segments: UserSegment[] = [
      ...
      },
      {
        ...
        criteria: {
          purchaseFrequency: { eq: 0 },
          engagementScore: { gt: 0.5 },
        },
        userCount: 0,
  ...
  private calculateAverageSessionDuration(activityData: any[]): number {
    ...
    activityData.forEach((activity) => {
      ...
      const sessionKey =
        activity.sessionId || `${activity.userId}_${timestamp.toDateString()}`;
  ...
  private isRepeatCustomer(purchases: any[]): boolean {
    ...
    const daysBetween =
      (Date.now() - sortedPurchases[0].createdAt.getTime()) /
      (1000 * 60 * 60 * 24);
  ...
  private isPriceSensitive(purchases: any[], activityData: any[]): boolean {
    ...
    const couponUsage = activityData.filter(
      (a) => a.entityType === 'coupon',
    ).length;
    const dealViews = activityData.filter(
      (a) => a.entityType === 'deal',
    ).length;
  ...
  private async analyzeBehavioralPatterns(
    ...
    const weekendActivities = activityData.filter((a) =>
      [0, 6].includes(a.createdAt.getDay()),
    );
    ...
    const mobileActivities = activityData.filter(
  ...
  private async calculatePurchaseMetrics(
    userId: string,
  ): Promise<{ frequency: number; avgOrderValue: number }> {
    const purchases = await this.getUserPurchases(userId);
  ...
  private async analyzeSessionMetrics(
    userId: string,
    activityData: any[],
  ): Promise<UserProfile['sessionMetrics']> {
    ...
    activityData.forEach((activity) => {
      const sessionKey =
        activity.sessionId ||
        `${activity.userId}_${activity.createdAt.toDateString()}`;
      sessions.set(sessionKey, (sessions.get(sessionKey) || 0) + 1);
    });

    const pagesPerSession =
      sessions.size > 0
        ? Array.from(sessions.values()).reduce((sum, count) => sum + count, 0) /
          sessions.size
        : 0;
    ...
    const bounceRate =
      sessions.size > 0 ? singlePageSessions / sessions.size : 0;
  ...
  async getProfilingAnalytics(): Promise<any> {
    ...
    return {
      ...
      averageEngagement:
        Array.from(this.profileCache.values()).reduce(
