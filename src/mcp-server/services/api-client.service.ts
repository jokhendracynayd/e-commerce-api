...
export class APIClientService {
  ...
  async getProductsByCategory(categorySlug: string, params?: Partial<SearchParams>): Promise<ApiResponse<Product[]>> {
    const queryParams = new URLSearchParams();
    if (params?.page) queryParams.append('page', params.page.toString());
    if (params?.limit) queryParams.append('limit', params.limit.toString());
    if (params?.sortBy) queryParams.append('sortBy', params.sortBy);
  ...
  async getProductsByBrand(brandSlug: string, params?: Partial<SearchParams>): Promise<ApiResponse<Product[]>> {
    const queryParams = new URLSearchParams();
    if (params?.page) queryParams.append('page', params.page.toString());
    if (params?.limit) queryParams.append('limit', params.limit.toString());
    if (params?.sortBy) queryParams.append('sortBy', params.sortBy);
  ...
  async getUserOrders(userId: string, page: number = 1, limit: number = 10): Promise<ApiResponse<Order[]>> {
    return this.makeRequest<Order[]>({
      method: 'GET',
  ...
  async getOrderAnalytics(startDate?: string, endDate?: string): Promise<ApiResponse<any>> {
    const queryParams = new URLSearchParams();
  ...
  async checkBulkAvailability(productIds: string[]): Promise<ApiResponse<InventoryItem[]>> {
    return this.makeRequest<InventoryItem[]>({
      method: 'POST',
  ...
  async getUserActivity(userId: string, limit: number = 50): Promise<ApiResponse<any[]>> {
    return this.makeRequest<any[]>({
      method: 'GET',
  ...
  async getPopularProducts(limit: number = 10, period: string = '7d'): Promise<ApiResponse<Product[]>> {
    return this.makeRequest<Product[]>({
      method: 'GET',
  ...
  async getSearchSuggestions(query: string, limit: number = 5): Promise<ApiResponse<string[]>> {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      const response: AxiosResponse<ApiResponse<T>> = await this.httpClient.request(config);
      
      return response.data;
    } catch (error: any) {
      // Handle authentication errors
      if (error.response?.status === 401 && retryCount === 0) {
        this.logger.warn('Authentication failed, refreshing token and retrying');
        // Force token refresh
      ...
      if (this.isRetryableError(error) && retryCount < maxRetries) {
        ...
        this.logger.warn(`Retrying request in ${delay}ms (attempt ${retryCount + 1}/${maxRetries})`);
