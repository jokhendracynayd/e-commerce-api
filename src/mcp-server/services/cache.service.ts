...
export class MCPCacheService {
  ...
  constructor(private readonly mcpConfig: MCPConfigService) {
    ...
    this.logger.log(`MCP Cache initialized with TTL: ${this.defaultTTL}ms, Max Size: ${this.maxSize}`);
  }
  ...
  set<T>(key: string, value: T, ttl?: number): void {
    ...
    this.logger.debug(`Cache set: ${key} (expires at ${new Date(expiry).toISOString()})`);
  }
   ...
   */
  static generateUserKey(userId: string, prefix: string, params: Record<string, any>): string {
  ...
  invalidateByPattern(pattern: string): number {
    ...
    if (deletedCount > 0) {
      this.logger.log(`Invalidated ${deletedCount} cache items matching pattern: ${pattern}`);
    }
